...
class ModelV4(ModelClass):
    ...
    def train(self):
        ...
        train_bin_loss = []
        train_bin_batches = np.array_split(self.DATA["TRAIN"], len(self.DATA["TRAIN"]) // self.CONFIG['batch_size'])
        ...
        for bn in range(len(train_bin_batches)):
            ...
            feed_dict_bin = {
                ...
                "img_input_best:0": self.DATA["IMG"][batch_train_bin[:, [2]][:,0],:],
                "rest_input_worst:0":  batch_train_bin[:, [3]],
                "img_input_worst:0": self.DATA["IMG"][batch_train_bin[:, [4]][:,0],:]
            }
    ...
    def dev(self):
        ...
        for batch_di in np.array_split(self.DATA["DEV"], 10):
            ...
            feed_dict_bin = {
                ...
                "img_input_best:0": self.DATA["IMG"][batch_dm[:, [2]][:,0],:]
            }
    ...
    def getFilteredData(self,verbose=True):
        ...
        '''
        IMG_F = pd.read_pickle(self.PATH + "img-food.pkl")
        IMG["vector_f"] = IMG_F["vector"]

        #IMG["vector"] = IMG[["vector","vector_f"]].apply(lambda x : x[0]+x[1], axis=1) # SUM
        #IMG["vector"] = IMG[["vector","vector_f"]].apply(lambda x : np.mean([x[0],x[1]], axis=0), axis=1) # AVG
        #IMG["vector"] = IMG[["vector","vector_f"]].apply(lambda x : np.concatenate((x[0],x[1])), axis=1) # CONCAT

        IMG = IMG.drop(columns=["vector_f"])
        '''
    ...
    def getData(self):
        ...
        if (os.path.exists(file_path) and len(os.listdir(file_path)) == 7):
            ...
            return {"TRAIN": TRAIN, "TRAIN_DEV": TRAIN_DEV, "DEV": DEV,"TEST": TEST, "IMG": IMG, "URLS": URLS, "N_RST": REST_TMP,"N_USR": USR_TMP, "V_IMG": V_IMG}
        ...
        return {"TRAIN": TRAIN, "TRAIN_DEV": TRAIN_DEV, "DEV": DEV,"TEST": TEST, "IMG": IMG, "URLS": URLS, "N_RST": len(REST_TMP),"N_USR": len(USR_TMP), "V_IMG": len(IMG.iloc[0].vector)}