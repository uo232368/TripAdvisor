...
class ModelV4_1_DEEP(ModelV4):
    ...
    def __init__(self,city,option,config,date,seed = 2):
        ...
        self.MODEL_NAME = "modelv4_1_deep"
    ...
    def getModel(self):
        ...
        with graph.as_default():
            ...
            E1 = tf.Variable(tf.truncated_normal([self.N_USR, emb_size*2], mean=0.0, stddev=1.0 / math.sqrt(emb_size*2)),name="E1")
            ...
            E3 = tf.Variable(tf.truncated_normal([self.V_IMG, emb_size], mean=0.0, stddev=1.0 / math.sqrt(emb_size)),name="E3")
    ...
    #-------------------------------------------------------------------------------------------------------------------
            #img_input_best = tf.nn.dropout(img_input_best,keep_prob=self.CONFIG['dropout'])
            #img_input_worst = tf.nn.dropout(img_input_worst,keep_prob=self.CONFIG['dropout'])
    # -------------------------------------------------------------------------------------------------------------------
            ...
            rest_img_emb = tf.concat([rest_emb,img_emb_best],axis=1, name="rest_img_emb")
            rest_img_emb_worst = tf.concat([rest_worst_emb,img_emb_worst],axis=1, name="rest_img_emb_worst")

            # Cálculo de LOSS y optimizador ----------------------------------------------------------------------------------------------

            dot_best = tf.reduce_sum(tf.multiply(user_emb, rest_img_emb), 1, keepdims=True, name="dot_best")
            dot_worst = tf.reduce_sum(tf.multiply(user_emb, rest_img_emb_worst), 1, keepdims=True, name="dot_worst")
